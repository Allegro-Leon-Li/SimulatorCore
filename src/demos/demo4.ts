...
function main() {
  ...
  ForwardButtonEl.addEventListener("click", () => {
    simulator.setPhysicsActive(true);
  ...
  PauseButtonEl.textContent = "Halt";
  PauseButtonEl.addEventListener("click", () => {
    const active = PauseButtonEl.textContent === "Resume";
    simulator.setPhysicsActive(active);
    PauseButtonEl.textContent =
      PauseButtonEl.textContent === "Halt" ? "Resume" : "Halt";
  });
